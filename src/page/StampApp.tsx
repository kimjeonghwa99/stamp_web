...
/* ================= QR 스캐너(재사용) =================
   ✅ 흰화면 방지: html5-qrcode는 동적 import로만 로드
*/
function QrScanner({
  elementId,
  title,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      console.error(e);
      alert("QR 스캐너 로딩 실패: html5-qrcode 설치/권한 확인");
    });
    ...
    setProgress((prev) =>
      prev.map((p) =>
        p.eventId === selectedEvent.id && p.couponToken === parsed.raw
          ? { ...p, couponUsed: true, couponUsedAtPlaceId: activePlaceId, couponUsedAtISO: nowISO }
          : p
      )
    );
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.bg, padding: 20 }}>
      <div style={{ maxWidth: 420, margin: "0 auto" }}>
        <h1 style={{ textAlign: "center", color: theme.primary, marginTop: 8 }}>QR 스탬프 이벤트</h1>

        <div className="seg">
          <button className={tab === "관리자" ? "seg-on" : "seg-off"} onClick={() => setTab("관리자")}>
            관리자
          </button>
          <button className={tab === "유저" ? "seg-on" : "seg-off"} onClick={() => setTab("유저")}>
            유저
          </button>
        </div>
        ...
        {tab === "관리자" && (
          <>
            <div className="seg" style={{ marginTop: 10 }}>
              <button
                className={adminView === "가맹점" ? "seg-on" : "seg-off"}
                onClick={() => setAdminView("가맹점")}
              >
                가맹점
              </button>
              <button
                className={adminView === "쿠폰검증" ? "seg-on" : "seg-off"}
                onClick={() => setAdminView("쿠폰검증")}
              >
                쿠폰 검증
              </button>
            </div>
            ...
            {adminView === "가맹점" && (
              <>
                <h3 style={{ marginTop: 16 }}>가맹점 추가</h3>
                ...
                <div className="field">
                  ...
                  />
                  <button className="btn-primary" onClick={addPlace}>
                    추가
                  </button>
                </div>
            ...
            {adminView === "쿠폰검증" && (
              <>
                <h3 style={{ marginTop: 16 }}>직원용 쿠폰 검증 (매장별)</h3>
                ...
                <button className="btn-primary" style={{ marginTop: 12 }} onClick={() => setShowStoreScanner((v) => !v)}>
                  🏪 매장 QR 스캔
                </button>
                ...
                <button
                  ...
                  style={{ marginTop: 10, opacity: activePlaceId ? 1 : 0.6 }}
                  onClick={() => {
                    if (!activePlaceId) {
          ...
          <>
            {!isCompleted ? (
              <>
                ...
                <button className="btn-primary" style={{ marginTop: 16 }} onClick={() => setShowUserScanner((v) => !v)}>
                  📷 매장 QR 찍기
                </button>
                ...
                <div style={{ marginTop: 16 }}>
                  ...
                  />
                  <button className="btn-soft" style={{ width: "100%", marginTop: 10 }} onClick={stampByQr}>
                    스탬프 찍기
                  </button>
                </div>
              </>
